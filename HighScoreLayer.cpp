...
{
	_playerPopup = LayerColor::create(ccc4(0, 0, 255, 200));
	...
	MenuItemLabel* save = MenuItemLabel::create(Label::createWithTTF("Save", "Arial", 20), CC_CALLBACK_1(HighScoreLayer::_changePlayerDone, this));
	save->setPosition(ccp(50, 50));

	MenuItemLabel* cancel = MenuItemLabel::create(Label::createWithTTF("Cancel", "Arial", 20), CC_CALLBACK_1(HighScoreLayer::_cancelPopupPlayer, this));
	...
	EditBox* playerName = EditBox::create(CCSizeMake(230, 40), Scale9Sprite::create("input.png"));
	...
	playerName->setReturnType(ui::EditBox::KeyboardReturnType::DONE);
	playerName->setPosition(ccp(125, 150));
	playerName->setText(UserDefault::sharedUserDefault()->getStringForKey("player", "anonymous").c_str());
...
void HighScoreLayer::_changePlayerDone(Ref *ref)
{
	EditBox* playerBox = (EditBox*)_playerPopup->getChildByTag(kPopUpEditBoxTag);
	CCUserDefault* defaults = CCUserDefault::sharedUserDefault();
...
void HighScoreLayer::_cancelPopupPlayer(Ref *ref)
{
	if (_playerPopup->isVisible())
	{
		Menu* menu = (Menu*)_playerPopup->getChildByTag(kPopUpPlayerTag);
		Object* _objectMenu;
		for (auto child : menu->getChildren())
		{
			MenuItem* item = (MenuItem*)_objectMenu;
...
void HighScoreLayer::button2Callback(CCObject *pSender)
{
	if (!_playerPopup->isVisible())
	{
		...
		EditBox* playerBox = (EditBox*)_playerPopup->getChildByTag(kPopUpEditBoxTag);
		playerBox->setText(UserDefault::sharedUserDefault()->getStringForKey("player", "anonymous").c_str());
